...
def compare_colors(color1, color2, threshold=0):
    r, g, b = color1
    r2, g2, b2 = color2
    num = 0
...
def is_same(pict_1, pict_2, threshold=0):
    """
    :param pict_1:
    :param pict_2:
    :param threshold:
    :return:
    >>> pict_1 = load_image(choose_file())
    >>> is_same(pict_1, pict_1)
    True
    >>> is_same(pict_1, create_image(get_width(pict_1), get_height(pict_1)))
    False
    """
    if get_width(pict_1) == get_width(pict_2) and get_height(pict_1) == get_height(pict_2):
        ...
        for x, y, (pict_1_r, pict_1_g, pict_1_b) in pict_1:
            pict_2_r, pict_2_g, pict_2_b = get_color(pict_2, x, y)
            set_color(difference, x, y, create_color(abs(pict_1_r - pict_2_r), abs(pict_1_g - pict_2_g), abs(pict_1_b - pict_2_b)))
            if not compare_colors(get_color(difference, x, y), (0, 0, 0), threshold):
                return False
        return True
    return False
